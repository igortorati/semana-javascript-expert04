...
import getTemplate from "./templates/atendeeTemplate.js"

const imgUser = document.getElementById('imgUser')
const roomTopic = document.getElementById('pTopic')
const girdAttendees = document.getElementById('girdAttendees')
const girdSpeakers = document.getElementById('girdSpeakers')
...
export default class View {
    ...
    static renderAudioElemtn({ callerId, stream, isCurrentId }) {
    ...
    static _onClapClick(command) {
        return () => {
            ...
            const basePath = './../../'
            const handActive = 'assets/icons/hand-solid.svg'
            const handInactive = 'assets/icons/hand.svg'
    ...
    static _toggleMicrophoneIcon() {
        ...
        const innactiveMicClass = 'fa-microphone-slash'
        const activeMicClass = 'fa-microphone'

        const isInactiveMic = classes.includes(innactiveMicClass)
        if(isInactiveMic) {
            icon.classList.remove(innactiveMicClass)
            icon.classList.add(activeMicClass)
            return;
        }

        icon.classList.remove(activeMicClass)
        icon.classList.add(innactiveMicClass)
    }
